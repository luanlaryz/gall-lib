...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Application entrypoint | Bootstrap da aplicacao, composicao de registries, runtime e lifecycle. | Sim | P0 | `app` | Parcial | Medio | Registries, defaults, lifecycle, rollback de startup, probes locais e helper de invocacao curta agora existem; integracoes de transporte concretas continuam fora do escopo. |
...
| Workflow chain | Encadeamento sequencial de etapas com passagem de contexto e resultados. | Sim | P0 | `workflow` | Parcial | Medio | Cadeia sequencial, branching controlado, retry, hooks e historico local ja existem; ainda faltam suites de conformidade mais amplas e adapters adicionais. |
| Workflow branching | Desvio condicional entre caminhos alternativos de workflow. | Sim | P1 | `workflow` | Parcial | Medio | Desvios para `Step`, `End` e `Suspend` ja existem; a cobertura futura deve reforcar casos de integracao e paridade. |
| Workflow retry | Politica de repeticao para etapas falhas com limites e estrategia. | Sim | P1 | `workflow/retry` | Parcial | Medio | Retry por workflow e por step ja existe com historico local; ainda faltam cenarios de paridade mais amplos. |
| Workflow hooks | Hooks locais antes, durante e depois da execucao de workflows. | Sim | P1 | `workflow/hooks` | Parcial | Medio | Hooks locais, logging hook e recuperacao de panic ja existem; integracoes futuras podem ampliar tracing sem servico hospedado. |
| Workflow execution history | Registro observavel das transicoes, etapas e resultados do workflow. | Sim | P1 | `workflow/history` | Parcial | Medio | Historico local append-only ja existe, com retries e checkpoints; ainda faltam suites de conformidade mais amplas. |
| Logger | Logging do runtime e das operacoes principais. | Sim | P0 | `logging` | Parcial | Baixo | Fachada minima, logger local simples, helper de contexto e instrumentacao de `App`, `Agent` e `Workflow` ja existem; formatos/exporters remotos continuam fora do escopo. |
| Observability hooks locais | Hooks para metricas, tracing local e eventos de instrumentacao. | Sim | P1 | `observability` | Parcial | Medio | Hooks locais de `app`, `agent` e `workflow`, compartilhamento de logger e recuperacao de panic com diagnostico local agora existem; metricas/tracing externos seguem fora desta etapa. |
